...
public class Abcd : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        for (int i = 1; i < 4; i++)
        {
            ...
            arr[array_size / 2 + i].y = arr[i + array_size / 2 - 1].y - 1.5f;
            pol[i].x = 0.9f;
            pol[i].y = arr[i].y;
        }
        for (int i = 4; i < array_size / 2; i++)
        {
    ...
    {
        next.x = Random.Range(x1 - 2.52f, x1 + 2.52f);
        next.y = next.x + 2 * Random.Range(ball_length + extra_distance, 2f * ball_length);
        return next;
    }
